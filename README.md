...
- 一次性获取电脑当前屏幕，在显式点击模式中映射一次左键点击，随后自动刷新快照
...
手机与电脑连接到同一个可互访的局域网。首选方式是在电脑上运行 `Quicker Link` 动作，然后在 App 的连接页面点击“扫描配对码”。如果电脑有多个局域网地址，先在二维码窗口中选择与手机同网段的地址。二维码在本机内存中生成，不会上传，但其中包含连接验证码，请勿截图或分享。新安装默认会使用 Android Keystore 加密保存验证码；每次打开 App 后首次连接会自动同步全局与通用动作并确认配套能力，同一电脑短线重连不会重复同步。可以在连接前关闭“加密保存并自动连接”。
...
- “当前屏幕”只在点击时获取一张快照，不进行连续录屏；全屏预览可显式切换到点击模式，单击一次后会自动刷新画面。
- “文本”可以读取电脑剪贴板、清空编辑框，也可以把编辑框内容写入电脑剪贴板；连接页另保留向当前窗口快速粘贴的入口。
...
`v0.4.0-alpha.1` 是使用项目专用 Release 密钥签名的早期 prerelease APK，用于测试，不代表已达到稳定生产版质量。GitHub Release 不会发布调试签名或未签名 APK。

如需首次安装，请从 [Releases](https://github.com/shuimowang/QuickerLink/releases) 同时下载 `quicker-link-v0.4.0-alpha.1-release.apk` 和同名 `.sha256` 文件。首次安装时，Android 可能要求允许安装来自浏览器或文件管理器的应用。

安装前计算下载文件的 SHA-256，并与 `.sha256` 文件中的 64 位十六进制值逐字符比较：

```powershell
(Get-FileHash -Algorithm SHA256 -LiteralPath ".\quicker-link-v0.4.0-alpha.1-release.apk").Hash.ToLowerInvariant()
```

```bash
sha256sum --check quicker-link-v0.4.0-alpha.1-release.apk.sha256
```
...
- 配套 Quicker 动作不连接第三方服务；它在电脑本机生成配对信息、返回全局与通用动作目录，并响应已配对 App 明确发起的动作终止、一次性屏幕快照与点击、剪贴板读取、小文件传输和固定电脑控制请求。
- 手机不能浏览电脑目录，也不能提交任意电脑路径。“接收文件”必须由电脑端用户通过系统文件选择窗口确认；手机文件只写入电脑端本机选择并保存的接收目录。
- 屏幕点击只接受当前快照的一次左键点击，不提供连续画面、拖动、键盘输入或通用远程控制；过期画面必须刷新。
...
当前为专用 Release 密钥签名的早期 prerelease，聚焦局域网内稳定触发动作和前台小规模数据交换。暂不支持公网中转、后台常驻、连续屏幕查看、完整远程桌面、文件夹或大文件传输、图片直接粘贴、断点续传或后台自动同步；面板动作目录只在扫码首次连接或用户手动刷新时同步。
