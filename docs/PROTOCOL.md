...
The v9 companion action exposes a user-facing local-automation entry point for bounded text, notification, and file delivery. Only a computer-local Quicker trigger may use it; an external trigger is rejected so a connected phone cannot submit a desktop file path or impersonate local automation.

Text is copied to the phone clipboard when Android permits and is retained in the transfer editor. Notifications use the Android notification permission. A file is staged on the desktop, announced by validated metadata, and downloaded only after the phone user accepts the offer. The existing 64 MiB transfer state machine, checksum validation, temporary files, and single-task rule still apply. Delivery is best effort and requires a current authenticated v9 client. Accepted text, notification, and file-offer commands produce one low-volume receipt cue when the phone's ringer, notification volume, and interruption policy allow it; notification permission controls visual publication, not this receipt cue.

## Manual clipboard and lifecycle

The transfer workspace exposes explicit computer-clipboard read and write operations while its authenticated UI is in use. The Android client does not run a continuous phone or computer clipboard watcher. A user may build a computer-local Quicker clipboard-change automation that invokes the companion action with `sendText:{text}`; in that arrangement Quicker owns change detection and the Android client handles only each resulting bounded text delivery.

Background receive is enabled by default and remains user-disableable. An Android `connectedDevice` foreground service owns the service notification and observes the application-level connection manager, allowing normal lock-screen, app-switch, and Activity-recreation transitions to retain WSS and reconnect. On Android 13+, notification permission controls whether desktop notifications and receipt prompts are visible; denial does not disable WSS retention or the policy-respecting receipt cue. Incoming text is consumed by the application runtime, while the latest file offer is retained for later UI acceptance. The service has a Stop action, no boot receiver, and `START_NOT_STICKY` behavior; stopping the service, force-stopping the app, or losing the process interrupts immediate delivery, and accepting a file offer still requires the UI.
...
- Catalog, action-control, toolbox, clipboard-write, desktop-push, and desktop-window capabilities currently require strict generation v9 on both sides. Older or malformed generations are rejected before state is changed or a side effect is executed.
...
- A screen click is bound to the decoded snapshot currently displayed and is consumed after one left click. Opening the screen workspace immediately starts serialized replacement snapshots; a new bitmap replaces the previous one only after decoding succeeds, and the client waits at least 80 ms after a completed transfer before requesting the next frame. The computer bounds captures to a 1280-pixel long side and uses compressed JPEG data, so this is responsive snapshot monitoring rather than a video stream. The companion does not expose dragging, arbitrary keyboard input, or a general remote-control channel.
- Desktop-window discovery returns at most 48 Alt+Tab-style, activatable windows with bounded PNG icons. Owned popups are grouped under one representative desktop window, and non-activatable helper windows are excluded. Activation accepts only short-lived random tokens bound to the enumerated window and process; raw window handles are never accepted from Android.
