...
- A screen click is bound to one recent snapshot and consumed after one left click. Opening the screen workspace immediately starts serialized replacement snapshots; after each transfer completes, the client waits at least 220 ms before requesting the next frame. The computer bounds captures to a 1280-pixel long side and uses compressed JPEG data, so this is responsive snapshot monitoring rather than a video stream. The companion does not expose dragging, arbitrary keyboard input, or a general remote-control channel.
- Desktop-window discovery returns at most 48 Alt+Tab-style windows with bounded PNG icons. Activation accepts only short-lived random tokens bound to the enumerated window and process; raw window handles are never accepted from Android.
