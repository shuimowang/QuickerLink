...
The optional background connection is also default off. When enabled by the user, an Android `connectedDevice` foreground service owns a persistent notification and observes the application-level connection manager, allowing normal lock-screen, app-switch, and Activity-recreation transitions to retain WSS and reconnect. Incoming text and notifications are consumed by the application runtime, while the latest file offer is retained for later UI acceptance and can produce a background notification. The service has a Stop action, no boot receiver, and `START_NOT_STICKY` behavior; stopping the service or losing the process interrupts immediate delivery, and accepting a file offer still requires the UI.
