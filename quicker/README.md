...
- 在电脑端用户确认文件选择后向手机发送单个小文件，或把手机发来的文件保存到“下载 / Quicker Link”
...
本目录只保留透明背景的动作图标源文件。配套动作独立发布，其维护源码和完整 ActionItem2 导出不随 Android 开源仓库分发。当前 v6 动作已在 Quicker `2.1.4.0`、运行时 `10.0.9` 上验证。
...
屏幕、剪贴板和文件功能位于 App 的“传输”页，只在前台由用户主动触发。文件限制为 8 MiB；电脑到手机必须在 Windows 文件选择窗口中确认，手机到电脑只写入“下载 / Quicker Link”。
